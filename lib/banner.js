$.fn.THBanner = function(arg) {
    ...
    function AddShowClass(flag) {
        if (flag) {
            ...
            if (oldBanner) {
                ...
                if (textList) {
                    var arr = textList.toArray();
                    for (var i = 0, length = arr.length; i < length; i++) {
                        $(arr[i]).attr("class", "animated").addClass($(arr[i]).attr("data-classout"));
                    }
                }
            ...
            if (newBanner) {
                ...
                if (textList) {
                    var arr = textList.toArray();
                    for (var i = 0, length = arr.length; i < length; i++) {
                        $(arr[i]).attr("class", "animated").addClass($(arr[i]).attr("data-classin"));
                    }
                }
            }
        }
    ...
    function change() { // л  ¼ 
        ...
        var prevIndex = opt.index - 1,
            nextIndex = opt.index + 1;
        if (prevIndex < 0) {
            prevIndex = itemLen - 1;
        }
